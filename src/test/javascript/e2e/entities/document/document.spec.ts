...
describe('Document e2e test', () => {
  ...
  it('should create and save Documents', async () => {
    ...
    expect(await documentUpdatePage.getCreationDateInput()).to.contain(
      '2001-01-01T02:30',
      'Expected creationDate value to be equals to 2000-12-31'
    );
    expect(await documentUpdatePage.getUserCreationInput()).to.eq(
      'userCreation',
      'Expected UserCreation value to be equals to userCreation'
    );
    expect(await documentUpdatePage.getDownloadDateInput()).to.contain(
      '2001-01-01T02:30',
      'Expected downloadDate value to be equals to 2000-12-31'
    );
    expect(await documentUpdatePage.getUserDownloadInput()).to.eq(
      'userDownload',
      'Expected UserDownload value to be equals to userDownload'
    );
  ...
  it('should delete last Document', async () => {
    ...
    expect(await documentDeleteDialog.getDialogTitle()).to.eq('komportementalistApp.document.delete.question');
